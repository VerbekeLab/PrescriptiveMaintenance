...
class SCIGAN_Model:
    ...
    def generator(self, x, y, t, d, z, treatment_dosage_samples):
        with tf.variable_scope('generator', reuse=tf.AUTO_REUSE):
            ...
            G_shared = tf.layers.Dense(self.h_dim, activation=tf.nn.elu, name='shared')(inputs)
    ...
    #         patient_features_representation = tf.layers.dense(x, self.h_inv_eqv_dim, activation=tf.nn.elu)
    #
    ...
    def train(self, Train_X, Train_T, Train_D, Train_Y, verbose=False):
        ...
        D_combined_prob = tf.nn.sigmoid(D_dosage_logits)
        ...
        for it in tqdm(range(50000)):
            ...
            for kk in range(1):
                ...
                # Treatment -- not needed for maintenance:
                # idx_mb = sample_X(Train_X, self.batch_size)
                # X_mb = Train_X[idx_mb, :]
                # T_mb = np.reshape(Train_T[idx_mb], [self.batch_size, ])
                # D_mb = np.reshape(Train_D[idx_mb], [self.batch_size, ])
            ...
            if it % 1000 == 0 and verbose:
                D_dosage_loss_curr, G_loss_curr, = self.sess.run(
                    [D_dosage_loss, G_loss],
                    feed_dict={self.X: X_mb, self.T: treatment_one_hot,
                               self.D: D_mb[:, np.newaxis],
                               self.Treatment_Dosage_Samples: treatment_dosage_samples,
                               self.Treatment_Dosage_Mask: treatment_dosage_mask,
    ...
    def tune(self, x_train, t_train, y_train, x_val, t_val, y_val, batch_sizes, h_dims, h_inv_eqv_dims,
        ...
        for batch_size in batch_sizes:
            ...
            for h_dim in h_dims:
                ...
                for h_inv_eqv_dim in h_inv_eqv_dims:
                    params['h_inv_eqv_dim'] = h_inv_eqv_dim

                    for num_dosage_samples in num_dosage_samples_range:
                        params['num_dosage_samples'] = num_dosage_samples

                        model = SCIGAN_Model(params)
                        ...
                        with tf.Session(graph=tf.Graph()) as sess:
                            ...
                            pred = sess.run('inference_outcomes:0',
                                                feed_dict={'input_features:0': x_val,
                                                           'input_treatment_dosage_samples:0': np.tile(t_val, num_dosage_samples)[:, np.newaxis, :]})
                            ...
                            print('Parameters: ')
                            print('Batch size: ' + str(batch_size) + '\tHidden dim: ' + str(h_dim) + '\tInv eq dim: ' + str(
                                h_inv_eqv_dim) + '\tNum dosage: ' + str(num_dosage_samples))
        ...
        print('Inv eq dim: ' + str(self.h_inv_eqv_dim))
        print('Num dosage: ' + str(self.num_dosage_samples))
